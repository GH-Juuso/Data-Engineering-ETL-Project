...
CREDS_FILE = Path("../private/credentials.csv")
SILVER_REJECTED_DIR = Path("../data/silver/rejected")
...
for y in YEARS:
    path = BRONZE_DIR / f"services-{y}.csv.gz"
    ...
    print(f"{y}: clean={len(clean):>7}  rejected={len(rejected)}")
...
print(f"Wrote {len(silver_sql)} rows to {SILVER_SCHEMA}.{SILVER_TABLE}")
...
print(f"Rows in {SILVER_SCHEMA}.{SILVER_TABLE}: {n}")
...
if len(rejected_all):
    ...
    print(f"Wrote {len(rejected_out)} rejected rows to {SILVER_REJECTED_DIR}")
else:
    print("No rejected rows to write")
